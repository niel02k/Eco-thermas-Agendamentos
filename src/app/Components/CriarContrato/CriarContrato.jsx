...
import {Save, X, Plus, Trash2, UserPlus, CreditCard,FileText, AlertCircle, ArrowRight, ArrowLeft} from "lucide-react";
import styles from "./CriarContrato.module.css";
import { useConsultores } from "@/app/hooks/useConsultores";
import { useContratos } from "@/app/hooks/useContratos";
import { PAGAMENTO, PAGAMENTO_LABELS, COBRANCA, COBRANCA_LABELS, STATUS_CONTRATO, STATUS_CONTRATO_LABELS } from "@/lib/constats.js";
...
export default function CriarContrato({ onClose, onSuccess }) {
  const router = useRouter();
  const [step, setStep] = useState(1);
  const { consultores, loading: carregandoConsultores } = useConsultores();
  const { createContrato, loading, error, setError } = useContratos();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const contratoCriado = await createContrato(dadosContrato);
      
      setSucesso("Contrato criado com sucesso!");
      
      setTimeout(() => {
        if (onSuccess) onSuccess(contratoCriado);
        if (onClose) onClose();
      }, 1500);
